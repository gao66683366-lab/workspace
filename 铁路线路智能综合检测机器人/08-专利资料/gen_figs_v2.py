...
def cn(ax, x, y, text, **kw):
    kw.setdefault('fontfamily', 'Noto Sans CJK SC')
    ax.text(x, y, text, **kw)
...
cn(ax, 7, 7.5, '铁路线路智能视觉与多模态感知融合综合检测系统',
...
ax.add_patch(rect)
cn(ax, 0.5, 6.55, '感知层', fontsize=8, color='#555', va='top')
...
for label, detail, x, y in items:
    ...
    ax.add_patch(box)
    cn(ax, x+1.0, y+1.05, label, ha='center', va='top', fontsize=7.5, fontweight='bold')
    cn(ax, x+1.0, y+0.35, detail, ha='center', va='center', fontsize=6.5, color='#333')

# 计算层
rect = FancyBboxPatch((0.3, 3.6), 13.4, 1.4, boxstyle='round,pad=0.05',
                      facecolor='#E8F5E9', edgecolor='#90A4AE', linewidth=1.5)
ax.add_patch(rect)
cn(ax, 0.5, 4.85, '计算层', fontsize=8, color='#555', va='top')
calc = [
    ('工控机\nIP54防护', 1.0, 3.75),
    ('融合判定单元\n三级融合\n自适应动态加权', 5.2, 3.75),
    ('环形缓冲区\n帧边界对齐', 9.4, 3.75),
...
for label, x, y in calc:
    ...
    ax.add_patch(box)
    cn(ax, x+1.4, y+0.55, label, ha='center', va='center', fontsize=7.5, fontweight='bold')

# 供电层
rect = FancyBboxPatch((0.3, 2.1), 13.4, 1.2, boxstyle='round,pad=0.05',
                      facecolor='#F3E5F5', edgecolor='#90A4AE', linewidth=1.5)
ax.add_patch(rect)
cn(ax, 0.5, 3.15, '供电层', fontsize=8, color='#555', va='top')
box = FancyBboxPatch((0.5, 2.2), 13.0, 0.9, boxstyle='round,pad=0.08',
                     facecolor='#E1BEE7', edgecolor='#7B1FA2', linewidth=1.2)
ax.add_patch(box)
cn(ax, 7, 2.65, '供电模块  48V大容量蓄电池组  智能电源管理', ha='center', va='center', fontsize=8, fontweight='bold')

# 底部备注
cn(ax, 7, 1.4, '三网物理隔离：EtherCAT控制网 + 千兆采集网 + 无线传输网', ha='center', va='center', fontsize=8, color='#666')
cn(ax, 7, 1.05, '检测速度：0.5m/s（精细检测）/ 1m/s（常规检测）  |  覆盖轨面缺陷/道钉螺栓/焊缝/廓形/波磨/轨距/水平/高低共8项功能', ha='center', va='center', fontsize=7.5, color='#888')
cn(ax, 7, 0.65, '融合判定准确率97.5%  ·  传感器退化场景下仍保持96.1%  ·  空间对齐精度亚毫米级（平均偏差3.2mm，降低74.7%）', ha='center', va='center', fontsize=7.5, color='#888')
