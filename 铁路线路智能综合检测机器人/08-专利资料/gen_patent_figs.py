...
# 设置中文字体
plt.rcParams['font.sans-serif'] = ['Noto Sans CJK SC', 'SimHei', 'WenQuanYi Micro Hei', 'Arial Unicode MS']
...
def draw_box(ax, x, y, w, h, text, color='#E3F2FD', edgecolor='#1565C0', fontsize=9, bold=False):
...
def draw_circle(ax, cx, cy, r, text, color='#FFF3E0', edgecolor='#E65100', fontsize=8):
    circle = Circle((cx, cy), r, facecolor=color, edgecolor=edgecolor, linewidth=1.5)
    ax.add_patch(circle)
...
def gen_fig1():
    ...
    ax.set_ylim(0, 10)
    ax.axis('off')
    ax.set_title('图1 铁路线路综合检测系统总体架构图', fontsize=14, fontweight='bold', pad=15)
    ...
    ax.text(8, 6.8, '检测小车车体', ha='center', va='center', fontsize=11, fontweight='bold')
    ax.text(8, 6.3, '工控机 (IP54)', ha='center', va='center', fontsize=9)
    ax.text(8, 5.8, '三网物理隔离架构', ha='center', va='center', fontsize=8)
    ...
    for x, y, name, spec in sensors_left:
        draw_box(ax, x-1.2, y-0.6, 2.4, 1.2, f'{name}\n{spec}', '#E8F5E9', '#2E7D32', 8)
    ...
    for x, y, name, spec in sensors_right:
        draw_box(ax, x-0.1, y-0.6, 2.2, 1.2, f'{name}\n{spec}', '#E3F2FD', '#1565C0', 8)
    ...
    # 图例
    ax.text(0.3, 9.8, '图中：1—检测小车车体；2—2D工业相机；3—3D线激光传感器；4—HWT905姿态传感器；\n'
            '5—单点测距传感器；6—测距传感器矩阵；7—工控机；8—通信模块；9—编码器；10—供电模块',
            ha='left', va='center', fontsize=8, color='#616161')
...
def gen_fig2():
    ...
    ax.set_ylim(0, 8)
    ax.axis('off')
    ax.set_title('图2 三键索引时空对齐机制示意图', fontsize=14, fontweight='bold', pad=15)

    # 时间轴
    ax.annotate('', xy=(13, 7.2), xytext=(0.5, 7.2),
                arrowprops=dict(arrowstyle='->', color='black', lw=2))
    ax.text(7, 7.5, '时间轴 (t)', ha='center', va='center', fontsize=11, fontweight='bold')
    for i in range(5):
        x = 1.5 + i * 2.5
        ax.plot([x, x], [7.1, 6.9], 'k-', lw=1.5)
        ax.text(x, 6.7, f't{i}', ha='center', va='center', fontsize=9)

    # 帧编号行
    ax.annotate('', xy=(13, 6.2), xytext=(0.5, 6.2),
                arrowprops=dict(arrowstyle='->', color='#1565C0', lw=1.5))
    ax.text(7, 6.45, '帧编号 (FID)', ha='center', va='center', fontsize=10, color='#1565C0')
    for i in range(5):
        x = 1.5 + i * 2.5
        ax.plot([x, x], [6.1, 5.9], color='#1565C0', lw=1.5)
        ax.text(x, 5.7, f'FID_{i}', ha='center', va='center', fontsize=9, color='#1565C0')

    # 里程坐标行
    ax.annotate('', xy=(13, 5.2), xytext=(0.5, 5.2),
                arrowprops=dict(arrowstyle='->', color='#2E7D32', lw=1.5))
    ax.text(7, 5.45, '里程坐标 (s_k, m)', ha='center', va='center', fontsize=10, color='#2E7D32')
    里程_labels = ['0.000', '0.005', '0.010', '0.015', '0.020']
    for i in range(5):
        x = 1.5 + i * 2.5
        ax.text(x, 5.1, 里程_labels[i], ha='center', va='center', fontsize=9, color='#2E7D32')
...
def gen_fig3():
    ...
    ax.set_ylim(0, 10)
    ax.axis('off')
    ax.set_title('图3 自适应动态加权融合算法流程图', fontsize=14, fontweight='bold', pad=15)
...
def gen_fig4():
    ...
    ax.set_ylim(0, 8)
    ax.axis('off')
    ax.set_title('图4 快慢双速EKF融合架构图', fontsize=14, fontweight='bold', pad=15)
    ...
    ax.text(5.5, 6.3, '快回路', fontsize=11, fontweight='bold', color='#E65100',
    ...
    ax.text(10.5, 6.3, '慢回路', fontsize=11, fontweight='bold', color='#2E7D32',
...
def gen_fig5():
    ...
    ax1.set_ylim(0, 7)
    ax1.axis('off')
    ax1.set_title('轨距检测', fontsize=12, fontweight='bold', pad=10)
    ...
    ax1.text(5, 5.8, '轨距G = d_left + d_right', ha='center', va='center', fontsize=10, color='red')
    ...
    ax1.text(4.5, 0.5, '测距矩阵', ha='center', va='center', fontsize=8)
    ax1.text(5.5, 0.5, '测距矩阵', ha='center', va='center', fontsize=8)
    ...
    ax1.text(4.0, 2.5, 'd_left', ha='center', fontsize=9, color='#0288D1')
    ax1.text(5.9, 2.5, 'd_right', ha='center', fontsize=9, color='#0288D1')
    ax1.text(5, 1.7, '姿态无关测量\n横滚角解耦', ha='center', va='center', fontsize=8,
    ...
    ax2.set_ylim(0, 7)
    ax2.axis('off')
    ax2.set_title('水平检测', fontsize=12, fontweight='bold', pad=10)

    # 曲线轨道示意
    ax2.plot([1, 9], [5.0, 5.0], 'k-', lw=2)  # 外轨
    ax2.plot([1, 9], [3.8, 4.4], 'k-', lw=2)   # 内轨（超高）
    ax2.fill_between([1, 9], [5.0, 5.0], [3.8, 4.4], alpha=0.3, color='#90CAF9')

    # 超高差标注
    ax2.annotate('', xy=(9.2, 5.0), xytext=(9.2, 4.4),
                arrowprops=dict(arrowstyle='<->', color='red', lw=2))
    ax2.text(9.5, 4.7, 'Δh', ha='center', va='center', fontsize=12, color='red', fontweight='bold')

    # 姿态传感器
    ax2.add_patch(Rectangle((4.0, 1.0), 2.0, 1.5, facecolor='#66BB6A', edgecolor='#2E7D32', linewidth=1.5))
    ax2.text(5, 0.5, 'HWT905姿态传感器', ha='center', va='center', fontsize=8)
    ax2.annotate('', xy=(5, 2.5), xytext=(5, 5.0), arrowprops=dict(arrowstyle='-', color='#2E7D32', lw=1, ls='--'))
    ax2.text(5.3, 3.5, '横滚角θ_r', ha='center', va='center', fontsize=9, color='#2E7D32')

    ax2.text(5, 1.7, 'Δh = 1435×sin(θ_r)\n精度<0.4mm', ha='center', va='center', fontsize=8,
    ...
    ax3.set_ylim(0, 7)
    ax3.axis('off')
    ax3.set_title('高低检测', fontsize=12, fontweight='bold', pad=10)
