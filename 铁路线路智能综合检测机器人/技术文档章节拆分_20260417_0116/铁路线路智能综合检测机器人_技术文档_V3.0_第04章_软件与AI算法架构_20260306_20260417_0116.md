...
> 本节基于 IMU 传感器实测参数（陀螺仪 + 加速度计 + 卡尔曼滤波融合），详细阐述钢轨水平/高低不平顺的检测算法原理、核心公式与参数依据。

##### 3.3.7.1 检测原理概述

钢轨水平/高低不平顺的检测依赖**双陀螺仪姿态测量系统**，通过卡尔曼滤波融合加速度计（静态准）与陀螺仪（动态响应快）的各自优势，输出高精度的俯仰角（Pitch）和横滚角（Roll），再乘以轨距换算为左右轨的相对高差与水平偏差。

```mermaid
flowchart LR
    A["加速度计<br/>三轴加速度"] --> KF["卡尔曼滤波<br/>融合算法"]
    B["陀螺仪<br/>三轴角速度"] --> KF
    KF --> PITCH["俯仰角 θ_pitch<br/>精度 0.05°"]
    KF --> ROLL["横滚角 θ_roll<br/>精度 0.05°"]
    PITCH --> LH["高低不平顺 Δh<br/>Δh = G × sin θ_pitch"]
    ROLL --> LL["水平偏差 Δl<br/>Δl = G × sin θ_roll"]
    LH & LL --> JUD["超限判定<br/>触发告警"]
```

> **图 3.3.7-1：钢轨水平/高低检测算法流程图**

---

##### 3.3.7.2 IMU 传感器参数（实测依据）

本系统选用的 IMU 传感器各轴参数如下：
...
| 量程 | — | ±6g |
| 分辨率 | ±6g | 0.0005g / LSB |
| RMS 噪声 | 带宽=100Hz | 1.2mg rms |
| 静止零漂 | 水平放置 | ±20mg |
| 温漂 | -40°C~+125°C | ±0.09mg / °C |
| 带宽 | — | 5~256Hz |
| 零偏稳定性（10s平滑） | — | XY: 15μg，Z: 25μg |
| **零偏不稳定性（Allan方差）** | Allan 方差 | **XY: 4μg，Z: 6μg** |
...
| RMS 噪声 | 带宽=100Hz | 0.028~0.07°/s rms |
| 静止零漂 | 水平放置 | ±0.5~1°/s |
| 温漂 | -40°C~+85°C | ±0.005~0.015°/s / °C |
| 带宽 | — | 5~256Hz |
| 零偏稳定性（10s平滑） | — | 8°/h |
| **零偏不稳定性（Allan方差）** | Allan 方差 | **5°/h** |

**倾角测量参数（Pitch / Roll / Heading）：**

| 参数 | 条件 | 典型值 |
|---|---|---|
| 俯仰/横滚量程 | X: ±180°，Y: ±90° |
| 俯仰/横滚倾角精度 | — | **0.05°** |
| 俯仰/横滚分辨率 | 水平放置 | 0.0055° |
| 俯仰/横滚温漂 | -40°C~+85°C | ±0.1° |
| 航向量程 | Z轴 | ±180° |
| 航向精度（静态，6轴算法） | — | **0.5°**（动态有积分累计误差） |
| 航向分辨率 | 水平放置 | 0.0055° |

> **注【1】**：使用前须在测试环境进行磁场校准，远离磁干扰。
> **注【2】**：动态震动环境下存在积分累计误差，须以实际测试为准。

---

##### 3.3.7.3 静态倾角测量（加速度计重力矢量分解）

当机器人静止或低速匀速运行时，可利用加速度计测量重力加速度在各轴的投影分量，通过几何关系直接计算俯仰角和横滚角。

**加速度计重力矢量分解：**

设三轴加速度计输出的比力为 $(a_x, a_y, a_z)$，在静态条件下（仅受重力 $g$ 作用），满足：

$$a_x^2 + a_y^2 + a_z^2 \approx g^2$$

重力加速度在载体坐标系的投影：

$$a_z = g \cdot \cos\theta_{\text{pitch}} \cdot \cos\phi_{\text{roll}}$$

$$a_x = g \cdot \sin\theta_{\text{pitch}}$$

$$a_y = -g \cdot \cos\theta_{\text{pitch}} \cdot \sin\phi_{\text{roll}}$$
...
$$\phi_{\text{roll}} = \arctan\left( \frac{-a_y}{a_z} \right)$$

| 参数 | 说明 | 典型值 |
|---|---|---|
| $a_x$ | X轴加速度（纵向，行驶方向） | 实时采集 |
| $a_y$ | Y轴加速度（横向，左/右） | 实时采集 |
| $a_z$ | Z轴加速度（垂向，垂直轨道） | 实时采集 |
| $g$ | 重力加速度 | 9.80665 m/s² |
| $\theta_{\text{pitch}}$ | 俯仰角（纵向倾斜，正负交替） | -90° ~ +90° |
| $\phi_{\text{roll}}$ | 横滚角（横向倾斜） | -180° ~ +180° |

**静态测量精度分析：**

加速度计 XY 轴零偏不稳定性为 4μg，对应角度噪声：

$$\delta\theta_{\text{static}} \approx \arctan\left( \frac{4 \times 10^{-6} g}{g} \right) \approx 4 \times 10^{-6} \text{ rad} \approx 0.0008°$$

远优于 0.05° 的目标精度，静态条件下可忽略加速度计噪声影响。

---

##### 3.3.7.4 动态角速度积分（陀螺仪）

当机器人以 1m/s 运行时，需要通过陀螺仪实时追踪角度的快速变化。将陀螺仪测得的角速度 $\omega$ 对时间积分得到角度变化：

$$\theta(t) = \theta(t_0) + \int_{t_0}^{t} \omega(\tau) \, d\tau$$

离散化形式（采样周期 $T_s$，本系统 $T_s = 1/256 \approx 3.9\text{ms}$）：

$$\theta_n = \theta_{n-1} + \omega_n \cdot T_s$$

| 参数 | 说明 | 典型值 |
|---|---|---|
| $\omega$ | 陀螺仪角速度（°/s） | ±2000°/s 量程 |
| $T_s$ | 采样周期 | 约 3.9ms（256Hz） |
| 积分漂移率 | 陀螺仪零偏不稳定性 | 5°/h = 0.0014°/s |

**动态漂移问题：**
陀螺仪积分会累积零偏误差（漂移），长时间运行会导致角度偏差持续增加。陀螺仪零偏不稳定性 5°/h，若不校正，每分钟累积误差约 0.083°。

**解决方案**：采用卡尔曼滤波，以加速度计静态角度为参考基准，持续修正陀螺仪积分漂移。

---

##### 3.3.7.5 卡尔曼滤波融合算法

卡尔曼滤波（Kalman Filter）在每一步同时利用加速度计（静态准但动态噪声大）和陀螺仪（动态响应快但积分有漂移）的输出，通过预测-更新两步递归，输出最优角度估计。

**状态向量定义：**

$$\mathbf{x}_k = \begin{bmatrix} \theta_k \\ \dot{\theta}_k \end{bmatrix}$$

其中 $\theta_k$ 为角度（俯仰或横滚），$\dot{\theta}_k$ 为角速度（由陀螺仪测量）。

**状态转移方程（预测步）：**

$$\hat{\mathbf{x}}_{k|k-1} = \mathbf{F}_k \cdot \hat{\mathbf{x}}_{k-1|k-1}$$

$$\mathbf{F}_k = \begin{bmatrix} 1 & T_s \\ 0 & 1 \end{bmatrix}$$

**状态更新方程（融合加速度计观测）：**

$$\hat{\mathbf{x}}_{k|k} = \hat{\mathbf{x}}_{k|k-1} + \mathbf{K}_k \cdot \left( z_k - \mathbf{H}_k \cdot \hat{\mathbf{x}}_{k|k-1} \right)$$

其中：
- $z_k$：加速度计在第 $k$ 步计算出的角度（观测值）
- $\mathbf{H}_k = \begin{bmatrix} 1 & 0 \end{bmatrix}$（只观测角度，不观测角速度）
- $\mathbf{K}_k$：卡尔曼增益（由状态协方差和观测噪声方差实时计算）

**卡尔曼增益公式：**

$$\mathbf{K}_k = \mathbf{P}_{k|k-1} \cdot \mathbf{H}_k^T \cdot \left( \mathbf{H}_k \cdot \mathbf{P}_{k|k-1} \cdot \mathbf{H}_k^T + \mathbf{R}_k \right)^{-1}$$

其中 $\mathbf{P}$ 为状态协方差矩阵，$\mathbf{R}$ 为观测噪声协方差。

**融合权重策略：**

| 工况 | 加速度计权重 | 陀螺仪权重 | 说明 |
|---|---|---|---|
| 静止/低速 | 高（接近 1） | 低（接近 0） | 以加速度计角度为准，修正漂移 |
| 高速/振动 | 低 | 高（接近 1） | 以陀螺仪积分为主，避免加速度计噪声干扰 |
| 正常匀速 | 动态调整 | 动态调整 | 卡尔曼滤波自适应融合 |

**融合输出精度（实测指标）：**

| 轴向 | 融合后角度精度 | 说明 |
|---|---|---|
| X轴（俯仰 Pitch） | **0.05°** | 卡尔曼滤波融合后静态+动态综合精度 |
| Y轴（横滚 Roll） | **0.05°** | 同上 |
| Z轴（航向 Yaw） | **0.5°**（静态） | 动态有积分累计误差，需磁力计辅助校正 |

---

##### 3.3.7.6 钢轨高低/水平计算公式

融合后的姿态角乘以轨距，换算为左右钢轨的相对高差和水平偏差。

**基本换算关系：**

$$\Delta h = G \times \sin(\theta_{\text{pitch}}) \approx G \times \theta_{\text{pitch}} \quad (\theta \text{ 较小时})$$

$$\Delta l = G \times \sin(\phi_{\text{roll}}) \approx G \times \phi_{\text{roll}} \quad (\phi \text{ 较小时})$$

其中 $G = 1435\text{ mm}$（标准轨距）。

| 符号 | 含义 | 说明 |
|---|---|---|
| $\theta_{\text{pitch}}$ | 俯仰角（纵向倾斜） | 卡尔曼滤波输出，精度 0.05° |
| $\phi_{\text{roll}}$ | 横滚角（横向倾斜） | 卡尔曼滤波输出，精度 0.05° |
| $G$ | 轨距 | 1435 mm（标准轨距） |
| $\Delta h$ | 左右轨高低不平顺（相对高差） | 单位：mm |
| $\Delta l$ | 左右轨水平偏差（横向不平顺） | 单位：mm |

**精度传递分析：**

角度测量精度 0.05° 传递到高差测量的精度：

$$\delta(\Delta h) = G \times \cos(\theta) \times \delta\theta \approx 1435 \times \frac{0.05° \times \pi}{180} \approx 1.25 \text{ mm}$$

因此，基于本 IMU 的钢轨高低/水平测量精度约为 **±1.25mm**，满足《铁路线路修理规则》对轨距和高低偏差的检测要求。
...
$$|\Delta h| > h_{\text{threshold}} \quad \text{或} \quad |\Delta l| > l_{\text{threshold}}$$

根据《铁路线路修理规则》，典型告警阈值设置如下：

| 等级 | 高低不平顺阈值 $|\Delta h|$ | 水平偏差阈值 $|\Delta l|$ | 处理建议 |
|---|---|---|---|
| 一级（限速） | > 15mm（允许速度 >120km/h） | > 15mm | 立即限速，运行检查 |
| 二级（维修） | > 10mm | > 10mm | 24h 内安排养护 |
| 三级（计划） | > 5mm | > 5mm | 纳入养护计划 |
| 四级（监测） | > 2mm | > 2mm | 持续监测记录 |

> **注**：上述阈值为典型参考值，实际以铁路局《线路维修规则》和线路等级对应技术标准为准，可通过云平台远程调参。

**里程位置绑定：**

每次超限事件触发时，同步记录：

$$(\text{里程坐标 } S,\; \Delta h,\; \phi_{\text{roll}},\; \theta_{\text{pitch}},\; \text{时间戳 } t)$$
