...
| 随机误差 | 角度随机游走（ARW） | $Q_{\epsilon}=1.0$ | °/√h（Allan系数，非积分结果） | $\sigma_{\text{ARW}}(\tau)=\dfrac{Q_{\epsilon}}{2\sqrt{2}\sqrt{\tau}}$，$\tau=1\text{h} \Rightarrow 0.35°$；$\tau=1\text{min} \Rightarrow 0.046°$ |
| 随机误差 | 偏置不稳定性（BI） | $B=5$ | °/h（Allan偏差值，τ无关） | $\sigma_{\text{BI}}(\tau)=\dfrac{B}{\sqrt{3}}$（τ域平台区佰），$\tau=1\text{h} \Rightarrow 2.9°$；$\tau=1\text{min} \Rightarrow 0.048°$ |
...
$$\sigma_{\text{ARW}}(\tau) = \frac{Q_{\epsilon}}{2\sqrt{2}} \cdot \frac{1}{\sqrt{\tau}}$$

其中 $Q_{\epsilon}=1.0°/\sqrt{\text{h}}$ 是 ARW 的 Allan 方差系数（IEEE 647-2006 定义）。

> **注**：本式与 $\tau^{+1/2}$ 成正比的错误公式（$\sigma=Q_{\epsilon}\cdot\sqrt{\tau}$）是 Allan 方差分析中最常见的混淆。正确公式中 $\sigma$ 与 $\tau^{-1/2}$ 成反比，积分时间越长，单次测量的 Allan 标准差越小（白噪声平均效应）。

数值验证（$\tau=1\text{min}=1/60\text{h}$）：

$$\sigma_{\text{ARW}}(1\text{min}) = \frac{1.0°/\sqrt{\text{h}}}{2\sqrt{2}\sqrt{1/60\text{h}}} = \frac{1.0}{2\sqrt{2}}\sqrt{60} \approx 0.354 \times 7.75 \approx 0.046°$$
...
$$\sigma_{\text{dyn,1min}} = \sqrt{\sigma_{\text{ARW}}^2 + \sigma_{\text{BI}}^2} = \sqrt{0.046^2 + 2.9^2} \approx \sqrt{0.002 + 8.41} \approx 2.90°$$

**修正前后对比（重要）**：

| 误差源 | 修正前（错误公式） | 修正后（正确公式） |
|---|---|---|
| ARW 1min 积分 | $0.129°$（错误：$\sigma=Q_{\epsilon}\sqrt{\tau}$） | $0.046°$（正确：$\sigma=\frac{Q_{\epsilon}}{2\sqrt{2}\sqrt{\tau}}$） |
| BI 1min 积分 | $0.048°$（错误：$\sigma=\frac{B}{\sqrt{3}}\tau$） | $2.9°$（正确：平台区佰，与 $\tau$ 无关） |
| **合计** | $0.137°$（严重低估 BI） | **$2.90°$（BI 在平台区主导）** |

> **物理意义**：在 $\tau=1\text{min}$ 尺度上，BI 的 Allan 平台区贡献（$2.9°$）远大于 ARW 贡献（$0.046°$）。这是因为 BI 表征的是低频偏置波动，在短时积分内既不能被平均掉（佰区），其 Allan 佰值反而成为主要噪声源。**原版 $0.137°$ 的合计值严重低估了实际积分误差。**

**结论**：纯陀螺仪积分在 1min 后角度误差约 $2.9°$（BI 主导），远大于原版 $0.14°$ 的错误估算。卡尔曼滤波融合通过加速度计基准持续修正，使融合输出精度维持在 $< 0.05°$，凸显了融合算法的必要性。
