...
**（1）角度随机游走（ARW）Allan 方差**

ARW 噪声的 Allan 标准差随积分时间 $	au$ **按 $	au^{-1/2}$ 衰减**（非 $	au^{+1/2}$ 增长）：

$$\sigma_{\text{ARW}}(\tau) = \frac{Q_{\epsilon}}{\sqrt{\tau}} \quad \text{(IEEE 647-2006 标准定义)}$$

其中 $Q_{\epsilon}=1.0°/\sqrt{\text{h}}$ 是 ARW 的 Allan 方差系数。

> **重要说明（关于本系统 IMU 噪声指标的合理性）**：上述公式的量纲要求 $\tau$ 以**小时为单位**代入，同时 $Q_{\epsilon}$ 也应换算为对应单位。使用标准形式 $Q_{\epsilon}=1.0°/\sqrt{\text{h}}$ 时，代入 $\tau$（单位：h）数值：
> $$\sigma_{\text{ARW}}(\tau) = \frac{1.0°}{\sqrt{\tau_{\text[h]}}}$$
> 则 $\sigma_{\text{ARW}}(\tau=1\text{h}) = 1.0°$（佰区佰），$\sigma_{\text{ARW}}(\tau=1/60\text{h}) = \sqrt{60} \approx 7.75°$（佰外衰减区）。

数值验证（$\tau=1\text{min}=1/60\text{h}$）：

$$\sigma_{\text{ARW}}(1\text{min}) = \frac{1.0°}{\sqrt{1/60}} = 1.0 \times \sqrt{60} \approx 7.75°$$

> **与原版错误的对比**：原版 $\sigma = Q_{\epsilon} \cdot \sqrt{\tau} = 0.129°$（方向完全相反，量纲亦不符），上一轮修正版 $\sigma = \frac{Q_{\epsilon}}{2\sqrt{2}\sqrt{\tau}} = 0.046°$（仍为错误来源）。本版按 IEEE 647-2006 正式修正为 $7.75°$，佰区佰为 $1.0°$（对应 $Q_{\epsilon}$ 标称值）。

> **注**：$Q_{\epsilon}=1.0°/\sqrt{\text{h}}$ 对应典型低精度 MEMS 工业陀螺仪。若实际 IMU 选型为更低噪声系数的器件（$Q_{\epsilon} < 0.5°/\sqrt{\text{h}}$），Allan 偏差等比缩小，本节误差预算随之改善。

**（2）偏置不稳定性（BI）Allan 方差**

偏置不稳定性 $B=5°/h$ 是 Allan 标准差曲线**平台区的最小值**（Allan 斜率为 0，与 $\tau$ 无关，不存在线性累积）：

$$\sigma_{\text{BI}} = \frac{B}{\sqrt{3}} \approx \frac{5°/h}{\sqrt{3}} \approx 2.9° \quad (\tau=1\text{h} \sim 100\text{s} \text{ 平台区值）}$$

> **注**：$\sigma_{\text{BI}}$ 在平台区佰与 $\tau$ 无关，因此在 $\tau=1\text{min}$ 和 $\tau=1\text{h}$ 均为 $\approx 2.9°$，不存在"积分时间越长误差越大"的线性关系。

**（3）合成动态积分误差（1min）**

$$\sigma_{\text{dyn,1min}} = \sqrt{\sigma_{\text{ARW}}^2 + \sigma_{\text{BI}}^2} = \sqrt{7.75^2 + 2.9^2} \approx \sqrt{60.06 + 8.41} \approx 8.27°$$

**修正前后对比（重要）**：

| 误差源 | 修正前（原版错误公式） | 修正后（IEEE标准公式） |
|---|---|---|
| ARW 1min | $0.129°$（错误：$Q_{\epsilon}\sqrt{\tau}$，方向相反） | $7.75°$（正确：$Q_{\epsilon}/\sqrt{\tau}$，IEEE 647-2006佰区外衰减） |
| BI 1min | $0.048°$（错误：$B\tau/\sqrt{3}$，不应有$\tau$因子） | $2.9°$（正确：佰区佰，$B/\sqrt{3}$） |
| **合计** | $0.137°$（严重低估，差57倍） | **$8.27°$（ARW在短τ主导，BI在长τ主导）** |

> **物理意义（修正版）**：在 $\tau=1\text{min}$ 尺度上，ARW 的 Allan 偏差（$7.75°$）远大于 BI（$2.9°$），ARW 在短τ占主导，与佰区（$\tau$ 较长时）的规律相反。随着 $\tau$ 增大，ARW 贡献减小（$\tau^{-1/2}$ 衰减），BI 在佰区（$\tau = 1\text{h}$ 附近）成为主要贡献。**这说明：纯陀螺仪积分在短时（<10min）以 ARW 为主，在长时（>1h）以 BI 为主。原版 $0.137°$ 完全错误（方向相反 + 量纲错误）。**

**结论（修正版）**：纯陀螺仪积分在 1min 后角度误差约 $8.27°$（ARW 主导）。卡尔曼滤波融合通过加速度计基准（静态精度 $0.00023°$）持续修正动态积分漂移，使融合输出精度维持在 $< 0.05°$，远优于纯积分方案，凸显融合算法的必要性。本 IMU 的 Allan 参数（$Q_{\epsilon}=1.0°/\sqrt{\text{h}}$，$B=5°/\text{h}$）属于典型低精度工业级 MEMS 水准，若需更高姿态精度应选用低噪声器件。
...
$$\mathbf{P}_{\infty} = \begin{bmatrix} \sqrt{2\sigma_{\omega}^2 T_s R} & 0 \\ 0 & \sigma_{\omega}^2 T_s \end{bmatrix}$$

$$K_{\infty} = \begin{bmatrix} K_1 \\ K_2 \end{bmatrix} = \begin{bmatrix} \sqrt{\dfrac{2\sigma_{\omega}^2 T_s}{R}} \\ \dfrac{\sigma_{\omega}^2 T_s^2}{2R} \end{bmatrix}$$

**代入数值验证（使用修正后单位）**：

- 陀螺仪角度随机游走 $Q_{\epsilon}=1.0°/\sqrt{\text{h}}$，换算为白噪声谱密度：
  $\sigma_{\omega}^2 = \dfrac{Q_{\epsilon}^2}{2} = \dfrac{(1.0°/\sqrt{\text{h}})^2}{2}$，换算为 $\text{rad}^2/\text{s}$：$\sigma_{\omega}^2 = \dfrac{(1.0 \times \pi/180)^2}{3600 \times 2} \approx 1.68 \times 10^{-7} \; \text{rad}^2/\text{s}$
- $T_s = 0.04\text{s}$，$R = 5.3 \times 10^{-8} \; \text{rad}^2$

$$K_1 = \sqrt{\frac{2 \times 1.68 \times 10^{-7} \times 0.04}{5.3 \times 10^{-8}}} \approx 1.59$$

$$K_2 = \frac{1.68 \times 10^{-7} \times (0.04)^2}{2 \times 5.3 \times 10^{-8}} \approx 0.025$$

> **注**：上述 $K_1 \approx 1.6$（接近 1）说明稳态下对角度观测的信任度较高；$K_2 \approx 0.025$ 是角速度通道增益，数值较小。原文档 $K_1 \approx 0.04$ 为代入错误（单位不一致），应以本修正结果为准。
...
| 5 | 陀螺仪角度随机游走（ARW） | 1min Allan 偏差 $\sigma_{\text{ARW}}=0.046°$ | ° | $1.15$（未融合） |
